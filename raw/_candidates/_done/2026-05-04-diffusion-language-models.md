...
- type: paper
- summary: ML-GSAI (ICLR 2026) hybridize masked-diffusion with parallel AR decoding by elevating parallel decoding from token level to a higher slot level — diffusion-based slot selection plus autoregressive intra-slot infilling. 34% perf gain over prior MDMs, 18× speedup, and 2.33× speedup over strong AR baselines.
